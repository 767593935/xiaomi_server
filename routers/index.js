...
var  search_data=require("../data/search_data.json")
var  clasify_data=require("../data/classify_data.json")
// 引入jsonwebtoken
...
router.get('/search', function (req, res) {
  ...
  for(var i=0;i<search_data.length;i++){
    if(name){
      if(search_data[i].title.toLowerCase().indexOf(name)!=-1){
        datalist.push(search_data[i])
      }
    }else{
      datalist=search_data
    }
    if(datalist===[]){
      datalist.push({title:"没有你搜索的内容"})
    }
  }
  res.send({code:0,datalist})
}) 
// 首页加载
router.get('/firstView', function (req, res) {
  res.send({code:0,clasify_data})
}) 
